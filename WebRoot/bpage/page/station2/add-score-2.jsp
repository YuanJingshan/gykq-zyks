 ...
 * 分数确认提交
 */
function station2scoreConfirm() {
	var dataStr = "&id=" + <%=id%>;
	$(".score-count").each(function(){
		dataStr = dataStr + "&" + this.id + "=" + $(this).text();
	});
	dataStr = dataStr + "&count=" + $("#count").text();
	dataStr = $("#station2-score").serialize() + dataStr;
	$.ajax({
		type: "post",
		dataType: "json",
		url: "stationScoreServlet?method=addStation2Score",
		data: dataStr,
		...
		success: function(data){
			if (data.result == 1) {
				$.common.layer.msg("操作成功！",  {icon: 1, time: 2000}, function() {
					openPage("/doctorexam/bpage/page/station2/station-score-2.jsp","_self");
				});
			} else if (data.result == 2) {
				alert("加上第一站分数，总分已大于100，请检查！");
			} else {
				alert("录入分数失败！");
			}
		}
    ...
    <div class="body_right">
    	<div class="body_right_1">
            ...
            <div class="occupying_right_1">
            	...
            	<div class="clear"></div>
            	<form id="station2-score" method="post">
	            	...
	            	<div style="width:260px; margin-left:auto; margin-right:auto;" class="ty_bg_c_10">
	                    <a class="reset" href="javascript:void(0);" onclick="resetScore('station2-score');">重　置</a>
	                    <a class="submit" href="javascript:void(0);" onclick="station2scoreConfirm();">提　交</a>
	                </div>
                </form>
            </div>
            <div class="clear"></div>
